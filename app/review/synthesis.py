...
注意：
- 这里是**确定性输出**（不依赖 LLM），便于稳定回写 GitLab/GitHub
...
def synthesize_review_markdown_body(
    head_sha: str, plan: RiskPlan, comments: list[ReviewComment],
) -> str:
    ...
    if not comments:
        lines.append("未发现需要阻塞的明显问题（基于 diff + symbol 上下文）。")
        return "\n".join(lines)
